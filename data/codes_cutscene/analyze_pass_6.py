...
import bisect # Used for efficient chronological checking
...
def normalize_phrase_for_consecutive_search(text):
    return " ".join(text.lower().split())

# --- NEW: Helper for the Sixth Pass chronological check ---
def check_chronological_order(row_index, match_file_index, timeline_map, sorted_rows):
    if not sorted_rows:
        return "N/A" # No timeline context exists

    # Find the position of the current row index in the sorted list of matched rows
    pos = bisect.bisect_left(sorted_rows, row_index)
    
    # Get the file index of the last matched subtitle BEFORE this one
    last_known_index = timeline_map[sorted_rows[pos - 1]] if pos > 0 else -1
    # Get the file index of the first matched subtitle AFTER this one
    next_known_index = timeline_map[sorted_rows[pos]] if pos < len(sorted_rows) else float('inf')

    if last_known_index == -1 and next_known_index == float('inf'):
        return "N/A" # This is the only subtitle, no context
    
    # The new match is valid if its file index falls between the surrounding known indices
    if last_known_index <= match_file_index <= next_known_index:
        return "Yes"
    else:
        return "No"

# --- REWRITTEN Sixth Pass to find all hits and check chronology ---
def perform_consecutive_word_pass(unmatched_df, all_text_files, timeline_map, file_to_index):
    print("\nPerforming sixth pass (consecutive word, all hits) analysis...")
    results = {}
    
    sorted_timeline_rows = sorted(timeline_map.keys())

    print("  - Pre-caching normalized lines from all TXT files...")
    normalized_lines_cache = {
        tf['txt_filename']: [normalize_phrase_for_consecutive_search(line) for line in tf['content'].splitlines()]
        for tf in tqdm(all_text_files, desc="Caching TXT files")
    }

    for index, row in tqdm(unmatched_df.iterrows(), total=unmatched_df.shape[0], desc="Analyzing subtitles (consecutive words)"):
        subtitle_text = str(row['Unmatched Subtitle'])
        search_phrase = normalize_phrase_for_consecutive_search(subtitle_text)
        if not search_phrase: continue

        found_matches = []
        for txt_filename, normalized_lines in normalized_lines_cache.items():
            for line in normalized_lines:
                if search_phrase in line:
                    match_file_index = file_to_index[txt_filename]
                    chrono_status = check_chronological_order(row['Original Excel Row'], match_file_index, timeline_map, sorted_timeline_rows)
                    found_matches.append({'file': txt_filename, 'chrono': chrono_status})
                    break # Found in this file, move to the next file

        if found_matches:
            # Summarize the results for this subtitle
            is_valid = "Yes" if any(m['chrono'] == "Yes" for m in found_matches) else "No"
            if is_valid == "No" and all(m['chrono'] == "N/A" for m in found_matches):
                is_valid = "N/A"
                
            results[row['Original Excel Row']] = {
                'count': len(found_matches),
                'files': " | ".join(m['file'] for m in found_matches),
                'chrono_valid': is_valid
            }
    
    print("  - Sixth pass complete.")
    return results
...
def run_consecutive_pass_only(args):
    """Special mode to run only the sixth pass on a previous report."""
    print("--- Running in Standalone Consecutive Pass Mode ---")
    if not args.input_report or not args.txt_folder or not args.output_report:
        print("Error: For this mode, you must provide --input-report, --txt-folder, and --output-report.")
        return

    print(f"\nLoading data from '{args.input_report}' and '{args.txt_folder}'...")
    try:
        report_xls = pd.ExcelFile(args.input_report)
        unmatched_df = report_xls.parse('Unmatched Excel Subtitles')
        # Load successful matches to build the timeline
        successful_df = report_xls.parse('Successful Matches')
    except Exception as e:
        print(f"Error: Could not read the input report file. {e}")
        return
        
    text_files = load_text_files_data(args.txt_folder)
    if not text_files: return

    # Build the timeline map from the original report's successful matches
    print("  - Building chronological timeline from original report...")
    file_to_index = {tf['txt_filename']: i for i, tf in enumerate(text_files)}
    timeline_map = {
        row['Original Excel Row']: file_to_index.get(row['TXT Filename'], -1)
        for index, row in successful_df.iterrows()
    }

    results = perform_consecutive_word_pass(unmatched_df, text_files, timeline_map, file_to_index)

    print("\nAdding new columns to the report...")
    unmatched_df['Consecutive Match Count'] = unmatched_df['Original Excel Row'].map(lambda x: results.get(x, {}).get('count', 0))
    unmatched_df['Consecutive Match Files'] = unmatched_df['Original Excel Row'].map(lambda x: results.get(x, {}).get('files', 'N/A'))
    unmatched_df['Chronologically Valid'] = unmatched_df['Original Excel Row'].map(lambda x: results.get(x, {}).get('chrono_valid', 'N/A'))
    
    print(f"Saving updated report to '{args.output_report}'...")
    try:
        with pd.ExcelWriter(args.output_report, engine='openpyxl') as writer:
            for sheet_name in report_xls.sheet_names:
                if sheet_name != 'Unmatched Excel Subtitles':
                    report_xls.parse(sheet_name).to_excel(writer, sheet_name=sheet_name, index=False)
            
            unmatched_df.to_excel(writer, sheet_name='Unmatched Excel Subtitles', index=False)
        print("  - Successfully created updated report.")
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--run-consecutive-pass", action='store_true', help="[Standalone Mode] Run only the consecutive word pass on a previous report.")
    parser.add_argument("--input-report", type=str, help="[Standalone Mode] Path to the input report .xlsx file.")
    parser.add_argument("--output-report", type=str, help="[Standalone Mode] Path to save the new, updated report .xlsx file.")
    
    parser.add_argument("--txt-folder", help="Path to the folder with .txt files.")
    parser.add_argument("--xlsx-file", help="Path to the .xlsx file for analysis.")
    parser.add_argument("--subtract", type=int, default=364, help="A constant number to subtract from the .txt line count.")
    parser.add_argument("--output", type=str, default="full_analysis_report.xlsx", help="Path for the main output analysis report file.")
    ...
    parser.add_argument("--run-all-passes", action='store_true', help="[Shortcut] Run all optional passes (2, 3, 4, 5).")

    args = parser.parse_args()

    if args.run_consecutive_pass:
        run_consecutive_pass_only(args)
        return
    
    if not args.txt_folder or not args.xlsx_file:
        print("Error: For a full analysis, you must provide --txt-folder and --xlsx-file.")
        parser.print_help()
        return

    if args.run_all_passes:
        args.run_second_pass = True
        args.run_third_pass = True
        args.run_fourth_pass = True
        args.run_fifth_pass = True
    ...
    for um in unmatched_raw:
        ...
        unmatched_row = {
            ...
            'Best Match TXT Content': analysis_info.get('Best Match TXT Content', ''),
